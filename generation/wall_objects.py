...
class WallObjectGenerator:
    ...
    def generate_wall_objects(self, scene, use_constraint=True):
        ...
        scene.setdefault("wall_object_constraint_plan", {})
        ...
        if self.use_multiprocessing and self.pool_processes > 0:
    ...
    def generate_wall_objects_per_room(self, args):
        ...
        selected_wall_objects = selected_objects[room_id]["wall"]
        selected_wall_objects = self.order_objects_by_size(selected_wall_objects)
        wall_object_name2id = {
            item[0]: item[1] for item in selected_wall_objects
        }
        ...
        stored_plan = scene.get("wall_object_constraint_plan", {}).get(room_type)
        if self.constraint_type == "llm" and use_constraint:
            if stored_plan is not None:
                constraint_plan = stored_plan
            else:
                constraint_plan = self.llm(constraints_prompt)
                scene.setdefault("wall_object_constraint_plan", {})[room_type] = constraint_plan
        else:
            constraint_plan = ""
    ...
    def parse_wall_object_constraints(
        ...
        object2constraints = {}
        # Handle None or empty constraint text
    ...
    def get_initial_state(self, scene, doors, windows, room_vertices, open_walls):
        ...
        for object in scene["floor_objects"]:
            ...
            if room_poly.covers(object_center):
                object_height = (

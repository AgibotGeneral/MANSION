...
class WindowGenerator:
    def __init__(self, llm):
        ...
        self.window_data = compress_json.load(
            os.path.join(MANSION_BASE_DATA_DIR, "windows/window-database.json")
        )
    ...
    def generate_windows(self, scene, additional_requirements_window):
        room_map = {self._room_key(room): room for room in scene["rooms"]}
        name_lookup: Dict[str, str] = {}
        for room in scene["rooms"]:
            key = self._room_key(room)
            if room.get("id"):
                name_lookup[room["id"]] = key
            name_lookup[key] = key
        organized_walls, available_wall_str = self.get_wall_for_windows(scene, name_lookup)
        room_list_str = "\n".join(f"- {key}: type={room.get('roomType')} name={room.get('id')}" for key, room in room_map.items())
        ...
        window_prompt += "\nRoom keys (use exact tokens when specifying 'room'; use 'exterior' for outside):\n"
        window_prompt += room_list_str + "\n"
        window_prompt += "\n".join(f"- {key}" for key in room_map.keys())
        ...
        windows = []
        window_ids = []
        rows = [row for row in raw_window_plan.split("\n") if "|" in row]
        room_with_windows = []
        for row in rows:
            # parse window plan
    ...
    def get_wall_for_windows(self, scene, name_lookup: Dict[str, str]):
        # configurable minimum exterior wall width (meters) for windows
        try:
            min_width = float(os.environ.get("HD_MIN_WINDOW_WALL_WIDTH", "1.0"))
        except Exception:
            min_width = 1.0
        debug = False
        try:
            debug = bool(int(os.environ.get("HD_DEBUG_WINDOWS", "0")))
        except Exception:
            debug = False
        ...
        available_walls = []
        excluded_narrow = 0
        excluded_door_reserved = 0
        excluded_interior = 0

        for wall in scene["walls"]:
            wid = wall.get("id")
            if "connect_exterior" not in wall:
                excluded_interior += 1
                if debug:
                    print(f"[window-generator:debug] skip {wid}: not exterior candidate")
                continue
            if wid in walls_with_door:
                excluded_door_reserved += 1
                if debug:
                    print(f"[window-generator:debug] skip {wid}: reserved by door")
                continue
            if float(wall.get("width", 0.0)) < min_width:
                excluded_narrow += 1
                if debug:
                    print(f"[window-generator:debug] skip {wid}: width {wall.get('width')} < {min_width}")
                continue
            available_walls.append(wall)

        organized_walls = {}
        for wall in available_walls:
            room_id = name_lookup.get(wall["roomId"], wall["roomId"])
        ...
        all_walls = len(scene.get("walls", []))
        print(
            f"[window-generator] total_walls={all_walls}, exterior_candidates={len(available_walls)}, rooms_with_options={len(organized_walls)}"
        )
        if debug:
            print(
                f"[window-generator:debug] min_width={min_width}m; excluded: interior={excluded_interior}, reserved_by_door={excluded_door_reserved}, too_narrow={excluded_narrow}"
            )
        if not organized_walls:
            print(
                f"[window-generator] Warning: no exterior walls wider than {int(min_width)}m remain once doors are reserved; skipping window placement."
            )

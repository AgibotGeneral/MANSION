...
from mansion.config.constants import MANSION_BASE_DATA_DIR, DEBUGGING
...
class FloorPlanGenerator:
    ...
    def parse_raw_plan(self, raw_plan):
        ...
        for room in parsed_plan:
            ...
            # Downstream generators expect room["vertices"] to describe the usable polygon.
            room["vertices"] = room["full_vertices"]
    ...
    def select_materials(self, designs, topk):
        ...
        # COMMENTED OUT: Allow same material to be reused for same design string across floors
        # This ensures consistent material selection: same design string -> same material
        ...
        for i in range(len(designs)):
            # Always use the first candidate material for consistent selection
            if len(candidate_materials[i]) == 0:
                # Fallback: should not happen, but keep for safety
                selected_materials.append(None)
            else:
...
class MaterialSelector:
    def __init__(self, clip_model, clip_preprocess, clip_tokenizer):
        materials = compress_json.load(
            os.path.join(MANSION_BASE_DATA_DIR, "materials/material-database.json")
        )
    ...
    def load_features(self):
        try:
            self.material_feature_clip = compress_pickle.load(
                os.path.join(
                    MANSION_BASE_DATA_DIR, "materials/material_feature_clip.pkl"
                )
            )
        except:
            ...
            for material in tqdm(self.selected_materials):
                image = self.preprocess(
                    Image.open(
                        os.path.join(
                            MANSION_BASE_DATA_DIR, f"materials/images/{material}.png"
                        )
                    )
                ).unsqueeze(0)
                with torch.no_grad():
        ...
        try:
            self.color_feature_clip = compress_pickle.load(
                os.path.join(MANSION_BASE_DATA_DIR, "materials/color_feature_clip.pkl")
            )
        except:
            ...
            compress_pickle.dump(
                ...
                os.path.join(
                    MANSION_BASE_DATA_DIR, "materials/color_feature_clip.pkl"
                ),
            )
